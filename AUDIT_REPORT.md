...
### M-22. [OPEN] — Manifest References Non-Existent Icon Files
**File:** `public/manifest.json` (lines 15-25)

The manifest references `/icon-192.png` and `/icon-512.png`, but these files don't exist in the `public/` directory. The only icon files present are `icon.svg`, `file.svg`, `globe.svg`, `next.svg`, `vercel.svg`, and `window.svg`.
...
### M-24. [OPEN] — No `robots.txt` or Sitemap
**Severity:** MAJOR — SEO
...
### M-27. [OPEN] — No Skip Navigation Link
**Files:** `src/app/layout.tsx`, `src/components/layout/BottomNav.tsx`

There's no "Skip to main content" link. Keyboard users must tab through all navigation links before reaching the main content.

**Impact:** Poor keyboard navigation experience.

### M-28. [OPEN] — Journal Page: `stripHtml` Uses `document.createElement` During Render
...
### m-9. [OPEN] — Reading Plans: No Way to Reset Progress
**File:** `src/app/(main)/bible/plans/page.tsx`

There's no "Reset progress" button. If a user wants to restart a reading plan, they must manually uncheck every day.
...
13. Create PWA icons -- Generate 192px and 512px PNG icons
14. Clean up service worker precache list
15. Add XML sitemap
16. Add per-page meta descriptions
17. Remove `userScalable: false` -- Allow accessibility zoom
18. Add skip navigation link
19. Fix `stripHtml` SSR crash risk
...
*End of audit. 62 issues found: 12 critical (all fixed), 28 major (21 fixed, 7 open), 22 minor (12 fixed, 10 open).*
